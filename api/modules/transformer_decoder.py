...
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # Meeran
device = torch.device("cuda") # Meeran
import copy
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, ingr_features, ingr_mask, incremental_state, img_features):
        ...
        if ingr_features is None:
            ...
            boolean_ingr_mask = (
                ingr_mask.bool() if ingr_mask is not None else None
            )  # Meeran
            x, _ = self.cond_att(
                query=x,
                key=ingr_features,
                value=ingr_features,
                key_padding_mask=boolean_ingr_mask,
                incremental_state=incremental_state,
                static_kv=True,
            )
            ...
            mask = torch.cat(
                (
                    torch.zeros(
                        img_features.shape[1], img_features.shape[0], dtype=torch.bool
                    ).to(device),
                    ingr_mask,
                ),
                1,
...
class DecoderTransformer(nn.Module):
    ...
    def forward(
        ...
        if ingr_mask is not None:
            # ingr_mask = (1 - ingr_mask.squeeze(1)).byte()  # Meeran
            ingr_mask = (1-ingr_mask.squeeze(1)).bool() # Meeran
    ...
    def upgrade_state_dict(self, state_dict):
        if isinstance(self.embed_positions, SinusoidalPositionalEmbedding):
            ...
            if "decoder.embed_positions._float_tensor" not in state_dict:
                state_dict["decoder.embed_positions._float_tensor"] = (
                    torch.FloatTensor()
                )
        return state_dict
