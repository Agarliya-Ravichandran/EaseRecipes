...
def mask_from_eos(ids, eos_value, mult_before=True):
    # mask = torch.ones(ids.size()).to(device).byte() # Meeran
    # mask_aux = torch.ones(ids.size(0)).to(device).byte() # Meeran
    mask = torch.ones(ids.size()).to(device).bool() # Meeran
    mask_aux = torch.ones(ids.size(0)).to(device).bool() # Meeran
